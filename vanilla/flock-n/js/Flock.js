function Flock(args) {
    ...
    this.center = _.map(this.dimensions, function (dimension) {
        return dimension / 2;
    });
    ...
    this.boids = _.map(_.range(this.count), function () {
        ...
        return {
            ...
            velocity: _.map(this.center, function () {
                var speed = Maths.randomWithin(this.minSpeed, this.maxSpeed);
...
Flock.prototype.updatePositions = function () {
    _.each(this.boids, function (boid) {
        _.times(boid.position.length, function (i) {
...
Flock.prototype.reCenter = function () {
    ...
    }, new Uint8Array(this.center.length));
    var centroid = _.map(sum, function (n) {
        return n / this.boids.length;
    }.bind(this));

    // find vector from centroid to center of canvas
    var difference = Maths.subtractVectors(this.center, centroid);
